...
const MaterialCard = ({ material, onEdit, onDelete, onView }) => {
  const getIcon = () => {
    switch (material.type) {
      ...
      case 'Video': return <MdOndemandVideo className="text-indigo-500" size={32} />;
      case 'Image': return <MdAttachFile className="text-emerald-500" size={32} />;
      default: return <MdLink className="text-purple-500" size={32} />;
    }
  };

  return (
    <div className="bg-white/60 backdrop-blur-2xl rounded-[48px] p-9 border border-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.02)] hover:shadow-2xl hover:shadow-indigo-500/10 transition-all duration-500 group flex flex-col relative overflow-hidden hover:-translate-y-2 h-full">
      ...
      <div className="absolute top-8 right-8 opacity-0 group-hover:opacity-100 transition-all flex gap-3 z-20">
         <button onClick={() => onEdit(material)} className="w-10 h-10 bg-white/80 backdrop-blur-xl text-slate-400 hover:text-indigo-600 rounded-2xl shadow-xl border border-white flex items-center justify-center transition-all hover:scale-110"><MdEdit size={18} /></button>
      ...
      <div className="w-16 h-16 rounded-[24px] bg-white shadow-lg flex items-center justify-center mb-8 shrink-0 relative z-10 group-hover:scale-110 transition-transform duration-500">
        {getIcon()}
      </div>
      
      <div className="flex-1 space-y-4 relative z-10">
        <h3 onClick={() => onView(material)} className="text-lg font-black text-slate-800 cursor-pointer hover:text-indigo-600 transition-colors line-clamp-2 leading-tight font-display">
          {material.title}
        </h3>
        
        <div className="flex flex-wrap gap-2">
          {(Array.isArray(material.course) ? material.course.slice(0, 1) : [material.course]).map(c => (
             <span key={c} className="bg-indigo-50 text-indigo-600 text-[8px] font-black uppercase tracking-[0.2em] px-3 py-1.5 rounded-lg border border-indigo-100/50">
              {c?.replace(/-/g, ' ')}
             </span>
          ))}
          <span className="bg-slate-50 text-slate-400 text-[8px] font-black uppercase tracking-[0.2em] px-3 py-1.5 rounded-lg border border-slate-200/20">B_{material.batch}</span>
        </div>
      ...
      <div className="mt-8 pt-7 border-t border-slate-100/50 flex gap-4 relative z-10">
        <button onClick={() => onView(material)} className="flex-1 py-4 bg-slate-900 text-white hover:bg-indigo-600 text-[9px] font-black uppercase tracking-[0.4em] rounded-[20px] transition-all shadow-xl shadow-indigo-100 flex justify-center items-center gap-3 group/btn active:scale-95">
...
const Materials = () => {
  ...
  const inp = "w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-[22px] font-black text-sm outline-none focus:ring-4 focus:ring-violet-400/5 focus:border-violet-600 transition-all placeholder:text-slate-300";
  const lbl = "text-[9px] font-black text-slate-400 uppercase tracking-widest ml-2 mb-2.5 block";
  ...
  return (
    <div className="w-full space-y-12 animate-in fade-in duration-700 pb-20 relative">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 relative z-10">
        <div>
          <h1 className="text-4xl font-black text-slate-800 tracking-tight font-display mb-1">Asset Vault</h1>
          <p className="text-slate-400 font-bold text-sm tracking-wide flex items-center gap-2">
            <span className="w-2.5 h-2.5 rounded-full bg-indigo-500 animate-pulse shadow-[0_0_10px_rgba(99,102,241,0.4)]"></span>
            Multi-Engine Knowledge Repository Synced
          </p>
        </div>
        <div className="flex items-center gap-4">
           <button onClick={fetchMaterials} className={`p-4 bg-white/80 backdrop-blur-xl rounded-2xl shadow-sm border border-slate-200/50 text-slate-400 hover:text-indigo-600 transition-all ${loading ? 'animate-spin' : ''}`}>
              <MdRefresh size={24} />
           </button>
           <button onClick={() => openModal('upload')} className="bg-slate-900 text-white px-8 py-4 rounded-2xl font-black text-[11px] uppercase tracking-[0.4em] shadow-xl shadow-indigo-100 hover:bg-indigo-600 transition-all flex items-center gap-3 active:scale-95">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 relative z-10">
         {[
           { label: 'Total Vectors', val: materials.length, icon: <MdFolder />, bg: 'bg-indigo-50', color: 'text-indigo-600' },
           { label: 'PDF Engines', val: materials.filter(m => m.type === 'PDF').length, icon: <MdPictureAsPdf />, bg: 'bg-rose-50', color: 'text-rose-600' },
           { label: 'Video Streams', val: materials.filter(m => m.type === 'Video').length, icon: <MdOndemandVideo />, bg: 'bg-purple-50', color: 'text-purple-600' },
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 relative z-10">
         <div className="relative col-span-2 group">
            ...
            <input placeholder="Search assets by identifier..." value={filter.search} onChange={e => setFilter({...filter, search: e.target.value})} className="w-full pl-16 pr-8 py-5 bg-white/60 backdrop-blur-xl border border-white/60 rounded-[28px] shadow-sm font-black text-sm outline-none focus:ring-8 focus:ring-indigo-400/5 transition-all shadow-inner" />
         </div>
         <div className="relative">
            <select value={filter.course} onChange={e => setFilter({...filter, course: e.target.value})} className="w-full px-8 py-5 bg-white/60 backdrop-blur-xl border border-white/60 rounded-[28px] shadow-sm font-black text-[10px] uppercase tracking-[0.2em] outline-none cursor-pointer hover:bg-indigo-50 transition-all appearance-none">
         ...
         <div className="relative">
            <select value={filter.type} onChange={e => setFilter({...filter, type: e.target.value})} className="w-full px-8 py-5 bg-white/60 backdrop-blur-xl border border-white/60 rounded-[28px] shadow-sm font-black text-[10px] uppercase tracking-[0.2em] outline-none cursor-pointer hover:bg-indigo-50 transition-all appearance-none">
      ...
      {loading && materials.length === 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 relative z-10">
           {[...Array(4)].map((_,i) => <div key={i} className="h-80 bg-white/20 backdrop-blur-3xl rounded-[48px] animate-pulse border border-white/40" />)}
        </div>
      ) : materials.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 relative z-10">
        ...
        <div className="flex flex-col items-center justify-center py-40 bg-white/40 backdrop-blur-2xl rounded-[64px] border border-white/60 shadow-sm relative z-10">
           ...
           <p className="text-2xl font-black text-slate-300 uppercase tracking-[0.4em]">Vault Empty</p>
        </div>
      )}
           ...
           <div className="bg-white rounded-[56px] shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[92vh] animate-in zoom-in-95 duration-300">
              <div className="p-12 border-b border-slate-50 flex justify-between items-center bg-[#F8FAFC]/50">
                 <div>
                    ...
                    <p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.3em]">Fill in the details below to sync assets</p>
                 </div>
                    ...
                    <div className="space-y-12">
                       <div className="flex items-center gap-8 p-10 bg-slate-50 rounded-[44px] border border-slate-50">
                          <div className="w-24 h-24 bg-white rounded-3xl flex items-center justify-center shadow-xl">
                             ...
                             {currentMaterial.type === 'Link' && <MdLink className="text-indigo-500" size={48} />}
                             {currentMaterial.type === 'Image' && <MdAttachFile className="text-emerald-500" size={48} />}
                          </div>
                          <div>
                             <h4 className="text-2xl font-black text-slate-900 tracking-tight leading-none mb-3">{currentMaterial.title}</h4>
                             <div className="flex gap-2">
                                <span className="text-[10px] font-black text-violet-600 uppercase tracking-widest bg-violet-50 px-3 py-1.5 rounded-lg">Batch {currentMaterial.batch}</span>
                             </div>
                          </div>
                       </div>
                       ...
                       <div className="space-y-4">
                          ...
                          <div className="p-10 bg-slate-50/50 rounded-[40px] border border-slate-100 font-bold text-slate-500 leading-relaxed text-sm">
                       ...
                       <div className="space-y-4">
                          {currentMaterial.type === 'Link' ? (
                             <a href={currentMaterial.links?.[0]} target="_blank" rel="noreferrer" className="w-full py-6 bg-slate-900 text-white rounded-[32px] font-black text-[11px] uppercase tracking-[0.4em] text-center hover:bg-violet-600 transition-all shadow-2xl flex items-center justify-center gap-3">Launch Link <MdKeyboardArrowRight size={20}/></a>
                          ) : (
                             <div className="space-y-3">
                                {currentMaterial.files?.map((f, i) => (
                                   <a key={i} href={`http://localhost:7001/${f}`} target="_blank" rel="noreferrer" className="w-full py-5 bg-white border border-slate-100 rounded-[28px] flex items-center justify-center gap-3 font-black text-[10px] uppercase tracking-widest text-slate-600 hover:bg-slate-900 hover:text-white transition-all group">
                    ...
                    <form onSubmit={handleSave} className="space-y-10">
                        <div className="space-y-8">
                           ...
                           <div>
                             ...
                             <div className="grid grid-cols-2 gap-4 p-6 bg-slate-50 rounded-[40px] border border-slate-50">
                                {COURSES.map(course => (
                                   <button type="button" key={course} onClick={() => toggleCourse(course)} className={`flex items-center gap-3 px-5 py-4 rounded-2xl border-2 transition-all text-left ${form.course.includes(course) ? 'bg-violet-600 border-violet-600 text-white shadow-xl' : 'bg-white border-white text-slate-400 hover:border-violet-100'}`}>
                                      <span className="text-[10px] font-black uppercase tracking-widest leading-none">{course.replace(/-/g, ' ')}</span>
                                   </button>
                                ))}
                             </div>
                          </div>
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="relative group">
                             <input type="file" multiple onChange={e => setFiles(Array.from(e.target.files))} className="w-full px-6 py-16 bg-slate-50 border-4 border-dashed border-slate-100 rounded-[48px] text-[10px] font-black uppercase text-slate-300 file:hidden cursor-pointer hover:bg-violet-50/50 hover:border-violet-200 transition-all text-center" />
                             <div className="absolute inset-0 flex items-center justify-center pointer-events-none text-slate-400 flex-col gap-4">
                                ...
                                <span className="font-black text-[10px] tracking-[0.4em] uppercase">{files.length > 0 ? `${files.length} FILES INJECTED` : "READY FOR INJECTION"}</span>
                             </div>
                          </div>
                       </div>

                        <button type="submit" className="w-full py-7 bg-slate-900 text-white rounded-[32px] font-black text-[12px] uppercase tracking-[0.5em] shadow-2xl hover:bg-indigo-600 transition-all transform active:scale-[0.98]">
