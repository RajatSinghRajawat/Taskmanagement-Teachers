    ...
    <div className="w-full space-y-12 animate-in fade-in duration-700 pb-20 relative">
      ...
      <div className="max-w-5xl mx-auto space-y-12 relative z-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
          <div>
            <h1 className="text-4xl font-black text-slate-800 tracking-tight font-display mb-1">Activity Stream</h1>
          ...
          <div className="flex items-center gap-4">
             <button onClick={() => fetchNotifications(true)} className={`p-4 bg-white/80 backdrop-blur-xl rounded-2xl shadow-sm border border-slate-200/50 text-slate-400 hover:text-indigo-600 transition-all ${refreshing ? 'animate-spin' : ''}`}>
                <MdRefresh size={24} />
             </button>
             <div className="flex bg-white/40 backdrop-blur-md p-1.5 rounded-[24px] border border-white/60 shadow-sm">
                <button onClick={markAllRead} className="px-6 py-3.5 text-indigo-600 hover:bg-indigo-50 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 transition-all">
                   <MdDoneAll size={20} /> Mark Read
                </button>
                <button onClick={deleteAll} className="px-6 py-3.5 text-rose-500 hover:bg-rose-50 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 transition-all">
        ...
        <div className="space-y-6">
          <AnimatePresence mode="popLayout">
            {loading && notifications.length === 0 ? (
              [...Array(4)].map((_, i) => (
                <div key={i} className="h-32 bg-white/20 backdrop-blur-3xl rounded-[48px] animate-pulse border border-white/40 shadow-sm" />
              ))
              ...
              notifications.map((note) => (
                <motion.div 
                  ...
                  key={note._id} 
                  className={`group p-8 rounded-[48px] border transition-all flex items-start gap-8 relative overflow-hidden ${note.isRead ? 'bg-white/40 border-slate-100/50 opacity-60' : 'bg-white/70 backdrop-blur-2xl border-white shadow-[0_8px_30px_rgba(99,102,241,0.05)] ring-1 ring-indigo-50'}`}
                >
                  ...
                  <div className="flex-1 relative z-10">
                    <div className="flex justify-between items-start mb-3">
                      <div className="space-y-1">
                        <h3 className={`text-xl font-black tracking-tight font-display ${note.isRead ? 'text-slate-500' : 'text-slate-800'}`}>
                          {note.title}
                        </h3>
                  ...
                  {!note.isRead && (
                    <div className="w-3 h-3 rounded-full bg-indigo-600 mt-5 shadow-xl shadow-indigo-500 animate-pulse relative z-10" />
                  )}
                  
                  <div className="opacity-0 group-hover:opacity-100 transition-all absolute right-8 bottom-8">
                     <button className="p-3 bg-white text-slate-300 hover:text-indigo-600 rounded-xl shadow-xl border border-slate-50 transition-all hover:scale-110">
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center py-40 bg-white/40 backdrop-blur-2xl rounded-[64px] border border-white/60 shadow-sm text-center relative z-10">
                ...
                <h3 className="text-2xl font-black text-slate-300 uppercase tracking-[0.4em]">Stream Clear</h3>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-4">All intelligence logs have been processed.</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
