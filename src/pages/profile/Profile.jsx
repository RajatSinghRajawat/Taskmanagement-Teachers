...
const Profile = () => {
  ...
  const inp = "w-full px-6 py-4 bg-white border border-slate-200 rounded-2xl font-bold text-sm text-slate-800 outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all placeholder:text-slate-300 disabled:bg-slate-50 disabled:text-slate-500 disabled:cursor-not-allowed";
  const lbl = "block text-[11px] font-black text-slate-400 mb-2 uppercase tracking-widest ml-1";
    ...
    <div className="w-full space-y-10 animate-in fade-in duration-500 pb-20 relative">
      ...
      <div className="max-w-5xl mx-auto space-y-10 relative z-10">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
          <div>
            <h1 className="text-3xl font-black text-slate-800 tracking-tight mb-1">My Identity Hub</h1>
            <p className="text-slate-400 font-bold text-xs tracking-wide">Secure Management of Faculty Credentials</p>
          </div>
          <div className="flex items-center gap-3">
             {!isEditing ? (
               <button onClick={() => setIsEditing(true)} className="px-6 py-3.5 bg-indigo-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-700 transition-all flex items-center gap-2 shadow-lg shadow-indigo-200">
                  <MdEdit size={18} /> Edit Profile
               </button>
             ) : (
               <button onClick={() => setIsEditing(false)} className="px-6 py-3.5 bg-slate-100 text-slate-600 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-200 transition-all flex items-center gap-2">
                  <MdClose size={18} /> Cancel Edit
               </button>
             )}
             <button onClick={() => setShowPasswordModal(true)} className="px-6 py-3.5 bg-white text-indigo-600 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-indigo-100 hover:bg-indigo-50 transition-all flex items-center gap-2 shadow-sm">
                <MdLockOutline size={18} /> Security Key
             </button>
             <button onClick={() => { localStorage.clear(); window.location.href = "/"; }} className="px-6 py-3.5 bg-rose-50 text-rose-600 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-rose-100 hover:bg-rose-600 hover:text-white transition-all flex items-center gap-2 shadow-sm">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
           ...
           <div className="lg:col-span-1 space-y-6">
              <div className="bg-white p-10 rounded-[40px] border border-slate-100 shadow-sm text-center relative group">
                 <div className="relative mx-auto w-full max-w-[200px] mb-8">
                    <div className="w-full aspect-square rounded-[48px] overflow-hidden bg-slate-50 border-4 border-white shadow-xl relative group-hover:shadow-indigo-200 transition-all duration-500 cursor-pointer">
                       <img 
                       ...
                       <div className="absolute inset-0 bg-slate-900/0 group-hover:bg-slate-900/40 backdrop-blur-[0px] group-hover:backdrop-blur-[2px] transition-all flex flex-col items-center justify-center pointer-events-none">
                          ...
                          <span className="text-[9px] font-black text-white opacity-0 group-hover:opacity-100 uppercase tracking-widest">View Image</span>
                       </div>
                    </div>
                    {isEditing && (
                      <label className="absolute bottom-4 right-4 p-3 bg-white text-indigo-600 rounded-2xl shadow-2xl border border-slate-100 cursor-pointer hover:scale-110 transition-transform">
                         <MdCameraAlt size={22} />
                 ...
                 <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-6">{profile.designation}</p>
                 ...
                 <div className="pt-6 border-t border-slate-50 flex justify-center gap-4 mb-6">
                    <div className="flex flex-col items-center">
                       ...
                       <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Assets</p>
                    </div>
                    <div className="w-px h-8 bg-slate-100"></div>
                    <div className="flex flex-col items-center">
                       <p className="text-xs font-black text-slate-800">128</p>
                       <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Reports</p>
                    </div>
                 </div>

                 {/* 💬 QUICK CHAT OPTION */}
                 <button onClick={() => toast.success("Chat Sync Active")} className="w-full py-4 bg-indigo-50 text-indigo-600 rounded-[20px] font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-3 hover:bg-indigo-600 hover:text-white transition-all border border-indigo-100 active:scale-95">
              ...
              <div className="bg-indigo-600 p-8 rounded-[40px] text-white shadow-xl relative overflow-hidden group">
                 ...
                 <div className="flex items-center gap-4 relative z-10">
                    ...
                    <div>
                       <h3 className="text-sm font-black tracking-tight leading-none mb-1">Identity Verified</h3>
                       <p className="text-[8px] font-black uppercase tracking-widest opacity-60">System Security Clearance</p>
                    </div>
                 </div>
              </div>
           </div>
           ...
           <div className="lg:col-span-2">
              <form onSubmit={handleUpdate} className="bg-white p-10 sm:p-12 rounded-[48px] border border-slate-100 shadow-sm space-y-10 relative overflow-hidden">
                 <AnimatePresence>
                   {!isEditing && (
                     <motion.div 
                        initial={{ opacity: 0 }}
                 ...
                 {isEditing && (
                   <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="pt-8 border-t border-slate-50 flex gap-4 relative z-20">
                      <button type="submit" disabled={updating} className="flex-1 py-5 bg-slate-900 text-white rounded-[24px] font-black text-[11px] uppercase tracking-widest shadow-xl hover:bg-indigo-600 transition-all flex items-center justify-center gap-3 active:scale-95 disabled:opacity-50">
                         {updating ? 'Saving Changes...' : <><MdSave size={20}/> Authorize Updates</>}
                      </button>
                      <button type="button" onClick={() => setIsEditing(false)} className="px-8 py-5 bg-slate-50 text-slate-400 rounded-[24px] font-black text-[10px] uppercase tracking-widest hover:bg-slate-100 transition-all flex items-center justify-center gap-2">
        ...
        {showImagePreview && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 bg-slate-900/90 backdrop-blur-xl">
             ...
             >
                ...
                <img src={currentImageUrl} className="w-full h-auto rounded-[48px] shadow-2xl border-4 border-white/20" alt="Full Preview" />
             </motion.div>
          </div>
        )}
      </AnimatePresence>
              ...
              <div className="p-10 sm:p-12">
                <div className="flex justify-between items-start mb-10">
                   <div>
                      ...
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Update system entry key</p>
                   </div>
                ...
                <form onSubmit={handlePasswordUpdate} className="space-y-8">
                   ...
                   <div className="pt-4">
                      <button type="submit" disabled={updating} className="w-full py-6 bg-slate-900 text-white rounded-[28px] font-black text-[12px] uppercase tracking-[0.3em] shadow-xl shadow-indigo-100 hover:bg-indigo-600 transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-3">
