...
const Dashboard = () => {
  ...
  const getGreeting = () => {
    ...
    return `${timeGreeting}, ${name} Sir`;
  };

  if (loading) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
      <div className="w-14 h-14 border-4 border-indigo-100 border-t-indigo-600 rounded-full animate-spin"></div>
      <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Synchronizing Live Intelligence...</p>
    </div>
  );
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div>
          ...
          <p className="text-slate-400 font-bold text-xs tracking-[0.1em] uppercase flex items-center gap-2">
            ...
            Faculty Intelligence System • Live Data Stream Active
          </p>
        </div>
           ...
           <div className="relative z-10 h-full flex flex-col justify-between">
              <div>
                 <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="text-3xl font-black text-slate-800 font-display tracking-tight">Intelligence Command</h3>
                 </div>
                 <p className="text-slate-400 text-sm font-bold max-w-md leading-relaxed">
                    Deploy new curriculum assets, assign complex tasks, and manage student enrollment from a single high-fidelity interface.
                 </p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-12">
                 ...
                 <button onClick={() => navigate('/materials')} className="p-6 bg-white border border-slate-100 text-slate-600 rounded-[28px] font-black text-[11px] uppercase tracking-widest flex items-center justify-center gap-3 hover:bg-slate-50 transition-all active:scale-95 shadow-sm">
                    <MdCloudUpload size={22} /> Upload Assets
                 </button>
              </div>
           </div>
        </div>
           ...
           <div className="space-y-6 flex-1 overflow-y-auto max-h-[320px] pr-2 custom-scrollbar">
              ...
              {data.notifications.length === 0 && (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                   ...
                   <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest">No Intelligence Feed Available</p>
                </div>
              )}
           </div>
